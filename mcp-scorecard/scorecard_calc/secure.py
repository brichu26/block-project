...
import base64
from main import MCPDataGenerator


# Add your GitHub token here
HEADERS = {"Authorization": f"token {GITHUB_TOKEN}"} if GITHUB_TOKEN else {}

THRESHOLDS = [3.0, 2.0, 1.0, 0.5, 0.0, -0.5, -1.0, -1.5]  # Thresholds are done based on z-score.
...
class MCPAnalyzer:
    ...
    def calculate_popularity_score(self, owner, repo, stars, downloads, stats):
        ...
        print("Stats are here", stats)
        print("Mean stars are here", stats["mean_github_stars"])
        print("Std stars are here", stats["std_github_stars"])
        print("Mean forks are here", stats["mean_forks"])
        print("Std forks are here", stats["std_forks"])
        print("Mean downloads are here", stats["mean_download_count"])
        print("Std downloads are here", stats["std_download_count"])
        
        if stars is None:
            star_score = 0
        else:
            z_score_stars = self.calculate_z_score(stars, stats["mean_github_stars"], stats["std_github_stars"])
        ...
        if forks is None:
            ...
            z_score_forks = self.calculate_z_score(forks, stats["mean_forks"], stats["std_forks"])
        ...
        if downloads is None:
            ...
            z_score_downloads = self.calculate_z_score(downloads, stats["mean_download_count"], stats["std_download_count"])
        ...
        if last_commit_days is None:
            ...
            z_score_commit_days = self.calculate_z_score(last_commit_days or 9999, stats["mean_commit_days"], stats["std_commit_days"])
    ...
    def analyze_servers(self, stats):
        ...
        try:
            with open(output_file, mode="a", newline='', encoding="utf-8") as f:
                ...
                for server in tqdm(servers_to_analyze, desc="Analyzing servers", unit="server"):
                    ...
                    popularity_data = self.calculate_popularity_score(owner, repo, stars, downloads, stats)
                    if popularity_data:
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join(os.getcwd(), "trial_servers.csv")  # Use the csv file you generate from main.py here.
    
    analyzer = MCPAnalyzer(csv_file)
    dataGenerator = MCPDataGenerator()
    stats = dataGenerator.calculate_statistics("enriched_repos_metadata.csv") #recieve the mean, median, std of the data with this function. Use the csv file achieved through enrich_csv in main.py    
    
    if analyzer.load_servers():
        analyzer.analyze_servers(stats)
